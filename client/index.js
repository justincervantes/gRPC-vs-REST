...
const grpcClient = new ExperimentService(
  "localhost:50051",
  grpc.credentials.createInsecure()
);

async function testREST(message) {
  const start = Date.now();
  const response = await axios.post("http://localhost:3000/data", { message });
  const timeTaken = Date.now() - start;
  console.log("REST: ${response.data.message} | Time taken: ${timeTaken}ms");
  return timeTaken;
}

function testGRPC(message) {
  return new Promise((resolve) => {
    const start = Date.now();
    grpcClient.GetData({ message }, (error, response) => {
      if (error) throw error;
      const timeTaken = Date.now() - start;
      console.log(`gRPC: ${response.message} | Time taken: ${timeTaken}ms`);
      resolve(timeTaken);
    });
  });
}
...
async function runTests() {
  ...
  for (let i = 0; i < 10; i++) {
    restTimes.push(await testREST(message));
    grpcTimes.push(await testGRPC(message));
  }

  const avgRestTime = restTimes.reduce((a, b) => a + b, 0) / restTimes.length;
  const avgGrpcTime = grpcTimes.reduce((a, b) => a + b, 0) / grpcTimes.length;

  console.log(`\nAverage REST time: ${avgRestTime} ms`);
  console.log(`Average gRPC time: ${avgGrpcTime} ms`);
}
